...
1. 作为构造函数执行，代表当前构造函数
2. 作为对象属性执行，代表当前对象
3. 作为普通函数执行，代表 window
4. call、apply、bind，可以改变 this 指向

## ● 函数声明与函数表达式有什么不同
- 函数声明：会进行函数声明提升，函数可以在函数声明之前调用
```js
foo()   // bar
function foo() {
    console.log('bar')
...
## ● call 和 apply 的作用与区别
都是 函数原型(`function.prototype`) 上的一个方法，都是用来修改函数中 `this` 指向问题
